...
class Node:
    ...
    def validate_block(self, block, validate_txns=False):
        ...
        if validate_txns:
            # Check block timestamps cannot be too far into the future
            assert block.timestamp - time.time() < DIFFICULTY_PERIOD_IN_SECS, "Block too far in the future"

            # Block timestamps must advance every block period
            height = max(len(self.blocks) - BLOCKS_PER_DIFFICULTY_PERIOD,0)
            assert block.timestamp > self.blocks[height].timestamp, "Block periods cannot got backward in time"
...
def mine_forever(public_key):
    ...
    while True:
        block_subsity = node.get_block_subsidy()
        fees = node.calculate_fees(node.mempool)
        coinbase = prepare_coinbase(public_key, block_subsity + fees)
        unmined_block = Block(
        ...
        if mined_block:
            logger.info("")
            logger.info(f"Mined a block w/ subsidy {mined_block.txns[0].tx_outs[0].amount}")
            with lock:
                node.handle_block(mined_block)

def mine_genesis_block(node, public_key):
    coinbase = prepare_coinbase(public_key, node.get_block_subsidy(), tx_id="abc123")
    unmined_block = Block(txns=[coinbase], prev_id=None, nonce=0, bits=INITIAL_DIFFICULTY_BITS, timestamp=1546383741.5890396)
    mined_block = mine_block(unmined_block)
