...
def prepare_coinbase(public_key, tx_id=None):
    ...
    tx_outs=[
        TxOut(tx_id, index=0, amount=BLOCK_SUBSIDY, public_key=public_key)
    ]
)
##########
# Mining # 
...
def read_message(s):
    message = b''
    # Our first 4 bytes signify message length
    ...
    def handle(self):
        ...
        if command == "connect":
            if peer not in node.pending_peers and peer not in node.peers:
                ...
                logger.info(f'(handhsake) Accepted "connect" request from "{peer[0]}"')
            ...
            if peer in node.pending_peers and peer not in node.peers:
                ...
                logger.info(f'(handhsake) connected to "{peer[0]}"')
                send_message(peer, "connect-response", None)
 
                # Requrest their peers
        ...
        if command == "peers-response":
            ...
            # find our most recent block that peer does not know about
            #  but which builds upon a block they do know about
            peer_block_ids = data
